...
type RemoveLeadButtonProps = {
  workspaceId: string;
  leadId: string;
  leadName: string;
};

export default function RemoveLeadButton({
  workspaceId,
  leadId,
  leadName,
}: RemoveLeadButtonProps) {
  ...
  const handleConfirm = () => {
    startTransition(async () => {
      const result = await removeLead(workspaceId, leadId);
      if (result?.ok) {
        handleOpenChange(false);
      } else {
        setFormError(result?.formError ?? "Couldn't delete the lead.");
      }
    });
  };
      ...
      <Popup open={open} onOpenChange={handleOpenChange}>
        <div className="px-6 pb-2 pt-6">
          ...
          <DialogDescription className="mt-1 text-[12.5px] leading-relaxed text-zinc-500 dark:text-zinc-400">
            ...
            ? Notes and stage history will be removed too. This can&apos;t be
            undone.
          </DialogDescription>
        </div>
