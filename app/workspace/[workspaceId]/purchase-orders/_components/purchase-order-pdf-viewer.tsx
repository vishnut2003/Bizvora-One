...
} from "./purchase-order-pdf-document";
...
export default function PurchaseOrderPdfViewer({ company, order }: Props) {
    ...
    (async () => {
      try {
        const blob = await pdf(
          <PurchaseOrderPdfDocument company={company} order={order} />,
        ).toBlob();
        if (cancelled) return;
        ...
        console.error("[PurchaseOrderPdfViewer] render failed", err);
        if (cancelled) return;
        setError(
          err instanceof Error
            ? err.message
            : "Couldn't render the purchase order PDF.",
        );
      }
    })();
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <iframe
        title={`Purchase Order ${order.number} preview`}
        src={url}
