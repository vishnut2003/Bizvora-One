...
export default function ReceiptPdfViewer({ company, receipt }: Props) {
    ...
    (async () => {
      try {
        const blob = await pdf(
          <ReceiptPdfDocument company={company} receipt={receipt} />,
        ).toBlob();
        if (cancelled) return;
        ...
        console.error("[ReceiptPdfViewer] render failed", err);
        if (cancelled) return;
        setError(
          err instanceof Error
            ? err.message
            : "Couldn't render the receipt PDF.",
        );
      }
    })();
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-zinc-100 bg-zinc-50/60 px-4 py-2.5 dark:border-zinc-800 dark:bg-zinc-950/40">
        <span className="text-[11.5px] text-zinc-500 dark:text-zinc-400">
          Preview of {receipt.number}
        </span>
        <a
          href={url}
          download={`${receipt.number.replace(/[^\w-]+/g, "_")}.pdf`}
      ...
      <iframe
        title={`Receipt ${receipt.number} preview`}
        src={url}
