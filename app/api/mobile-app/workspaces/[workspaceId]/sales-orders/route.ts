...
export const GET = withMobile(async (req, ctx: Ctx) => {
  ...
  return ok(await listVoucherDocs(SALES_ORDER_CONFIG, access, new URL(req.url)));
});
...
export const POST = withMobile(async (req, ctx: Ctx) => {
  ...
  const doc = await createVoucherDoc(SALES_ORDER_CONFIG, access, body);
  return ok({ salesOrder: doc }, 201);
});
