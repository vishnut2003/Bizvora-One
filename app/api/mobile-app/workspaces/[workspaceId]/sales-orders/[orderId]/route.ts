...
import { SALES_ORDER_CONFIG } from "../../../../_lib/finance-configs";
...
export const GET = withMobile(async (req, ctx: Ctx) => {
  ...
  return ok({ salesOrder: await getVoucherDoc(SALES_ORDER_CONFIG, access, orderId) });
});
...
export const PATCH = withMobile(async (req, ctx: Ctx) => {
  ...
  const doc = await updateVoucherDoc(SALES_ORDER_CONFIG, access, orderId, body);
  return ok({ salesOrder: doc });
});
...
export const DELETE = withMobile(async (req, ctx: Ctx) => {
  ...
  requireVoucherViewer(SALES_ORDER_CONFIG, access);
  requireObjectId(orderId);
  await deleteVoucherDoc(SALES_ORDER_CONFIG, access, orderId);
  return ok();
});
