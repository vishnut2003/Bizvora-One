...
export async function GET(request: NextRequest) {
  ...
  // Multi-tenant: OAuth runs against the workspace's own Meta app, so its
  // credentials must be saved (via the integrations card) before connecting.
  ...
  if (!appId) {
    ...
    url.searchParams.set("meta-ad", "missing_credentials");
    return Response.redirect(url, 302);
  }
  ...
  const authUrl = buildMetaAuthUrl(appId, state);
